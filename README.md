...
| `--bitwidth_choices`      | no       | `4 8 16 32`  | Candidate bit-widths for quantization                                                               |
| `--lr_model`              | no       | `1e-6`       | Learning rate for model parameters                                                                  |
| `--lr_alloc`              | no       | `1e-3`       | Learning rate for allocation logits                                                                 |
| `--lr_lambda`             | no       | `1e-2`       | Learning rate for Lagrangian multiplier                                                             |
| `--max_epochs`            | no       | `100`        | Maximum number of training epochs                                                                   |
| `--cf_batch_size`         | no       | `16`         | Training batch size for counterfactual pairs                                                        |
| `--reduction_perc`        | no       | `0.8`        | Target compression percentage                                                                       |
| `--beta`                  | no       | `0.1`        | Quantization consistency loss weight                                                                |
| `--tolerance`             | no       | `1e-2`       | Complementary slackness tolerance (eps-KKT)                                                         |
